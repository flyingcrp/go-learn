...
- [x] 统一 department/role repo 中的 `gorm.ErrRecordNotFound` 判断，使用 `errors.Is`。
...
- [x] 给用户模块加一个详情接口：`GET /v1/user/:id`。
- [x] handler 只取 `id` 和返回响应。
- [x] service 判断用户是否存在。
- [x] repo 查询数据库。
- [x] 复用 `internal/common/response` 返回统一响应。
...
- [x] 实现 trace 中间件：生成/复用 trace ID，存入 `request.Context`，附加到响应头。
...
- [x] `cmd/server/main.go` 里直接读 `os.Getenv`。
- [x] 环境变量缺少默认值。
- [x] 环境变量缺少启动时校验。
- [x] JWT secret 还没有统一配置入口。
- [x] server addr、shutdown timeout 已进入统一配置入口。

项目练习：

- [x] 新建 `internal/config/config.go`。
- [x] 定义 `Config` struct。
- [x] 启动时一次性加载并校验环境变量。
- [x] 把 JWT secret 放进去。
- [x] 把 DB、server addr、shutdown timeout 放进去。
- [x] `Load()` 返回 `(*Config, error)`。
- [x] main 函数只负责组装依赖，不到处读环境变量。
...
- [x] `AuthGuard` 写了但没挂到路由。
- [x] JWT secret 硬编码。
- [x] `strings.Split(...)[1]` 在 Authorization 为空或格式错误时会 panic。
- [x] 鉴权成功后还没有把用户信息写入请求上下文。

项目练习：

- [x] 修复 Authorization 解析。
- [x] 从配置读取 JWT secret。
- [x] 校验 token 签名、过期时间、签名算法。
- [x] 给需要鉴权的路由组挂上中间件。
- [x] 中间件解析成功后用 `context.WithValue` 写入用户信息（`MustGetUserInfo`）。
- [x] handler/service 不直接解析 JWT，只读取中间件产物。
...
- [x] `Limit`、`Offset`。
- [x] `Order`。
- [x] `Transaction(func(tx *gorm.DB) error { ... })`。
- [x] 区分”业务分页”和”数据库分页”。

项目练习：

- [x] 加用户列表接口：`GET /v1/user?page=1&page_size=20`。
- [x] handler 解析分页参数。
- [x] service 处理默认值和边界。
- [x] repo 使用 `Limit`、`Offset` 查询。
- [x] 响应里返回列表和分页信息。
- [ ] 如果注册要同时写多张表，再使用事务。
...
- [x] JWT 中间件（已完成）。
- [x] 登录和 JWT 签发。
- [x] trace 中间件：生成/复用 trace ID，统一响应头。
- [ ] 统一错误和响应。
- [ ] 接口调试与文档。
